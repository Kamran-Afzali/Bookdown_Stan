...
shrinkage_data <- tibble(
  ...
  estimated_intercept = alpha_group_est,
  lower_ci = alpha_group_lower,
  upper_ci = alpha_group_upper,
  sample_size = table(train_data$group_id)[1:n_groups]
)

p2 <- ggplot(shrinkage_data, aes(x = true_intercept, y = estimated_intercept)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  geom_pointrange(aes(ymin = lower_ci, ymax = upper_ci, size = sample_size), 
                  alpha = 0.7) +
  labs(title = "Random Intercepts: Shrinkage Toward Population Mean",
       subtitle = "Points show posterior means with 95% credible intervals",
       x = "True Group Intercepts", 
       y = "Estimated Group Intercepts",
       size = "Sample Size") +
  theme_minimal() +
  coord_fixed()
